...
from game2048.game_engine.game import Game, Move, PastGame, GameProgress


def create_board(some_array):
    return np.array(some_array, dtype='uint16')
...
def test_possible_and_impossible_moves(board, possible_moves, impossible_moves):
    game = Game()
    game.set_from_other(create_board(board))
...
def test_game_progression(initial_board, game_progresses):
    game = Game()
    game.set_from_other(create_board(initial_board))
    for game_progress in game_progresses:
        assert game_progress in game.possible_moves
